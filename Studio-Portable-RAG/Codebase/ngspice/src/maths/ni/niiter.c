...
NIiter(CKTcircuit *ckt, int maxIter)
{
    ...
    for(;;) {
        ...
        if (diag_nr_have_rhs && ngspice_diag_wants_nr()) {
            int sz = SMPmatSize(ckt->CKTmatrix);
            int k;
            double max_dx = 0.0;
