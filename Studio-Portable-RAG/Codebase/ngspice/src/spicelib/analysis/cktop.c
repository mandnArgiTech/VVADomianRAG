              ...
              long int continuemode, int iterlim)
{
    ...
    while ((!success) && (!failed)) {
        ...
        if (ngspice_diag_wants_gmin() && isfinite(ckt->CKTdiagGmin) && isfinite((double) iters)) {
            ngspice_diag_emit_gmin(ckt->CKTdiagGmin, converged == 0, iters);
        }
             ...
             long int continuemode, int iterlim)
{
    ...
    for (i = 0; i <= ckt->CKTnumGminSteps; i++) {
        ...
        if (ngspice_diag_wants_gmin() && isfinite(ckt->CKTdiagGmin) && isfinite((double) iters_g)) {
            ngspice_diag_emit_gmin(ckt->CKTdiagGmin, converged == 0, iters_g);
        }
               ...
               long int continuemode, int iterlim)
{
    ...
    fprintf (stderr, "Supplies reduced to %8.4f%% ", ckt->CKTsrcFact * 100);
    {
        ...
        if (ngspice_diag_wants_src() && isfinite(ckt->CKTsrcFact) && isfinite((double) iters0)) {
            ngspice_diag_emit_src_step(ckt->CKTsrcFact, converged == 0, iters0);
        }
    }
    ...
    if (converged != 0) {
        ...
        for (i = 0; i <= 10; i++) {
...

            {
                ...
                if (ngspice_diag_wants_gmin() && isfinite(ckt->CKTdiagGmin) &&
                        isfinite((double) iters_g2)) {
                    ngspice_diag_emit_gmin(ckt->CKTdiagGmin, converged == 0, iters_g2);
                }
            }
    ...
    if (converged == 0)
        do {
            ...
            if (ngspice_diag_wants_src() && isfinite(ckt->CKTsrcFact) && isfinite((double) iters)) {
                ngspice_diag_emit_src_step(ckt->CKTsrcFact, converged == 0, iters);
            }
            ...
            long int continuemode, int iterlim)
{
    ...
    for (i = 0; i <= ckt->CKTnumSrcSteps; i++) {
        ...
        if (ngspice_diag_wants_src() && isfinite(ckt->CKTsrcFact) && isfinite((double) iters_s)) {
            ngspice_diag_emit_src_step(ckt->CKTsrcFact, converged == 0, iters_s);
        }
        ckt->CKTmode = continuemode;
        if (converged != 0) {
            ckt->CKTsrcFact = 1;
