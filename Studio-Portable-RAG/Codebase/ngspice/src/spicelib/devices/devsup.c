...
DEVpnjlim(double vnew, double vold, double vt, double vcrit, int *icheck)
{
    ...
    if (*icheck && ngspice_diag_wants_pnjlim() && isfinite(vnew_orig) && isfinite(vnew) &&
            isfinite(vold) && isfinite(vcrit)) {
...
DEVfetlim(double vnew, double vold, double vto)
{
    ...
    if (vnew != vnew_orig && ngspice_diag_wants_fetlim() && isfinite(vnew_orig) &&
            isfinite(vnew) && isfinite(vold) && isfinite(vto)) {
        ngspice_diag_emit_limiter_fet("", vnew_orig, vnew, vold, vto);
    }
    return(vnew);
}
