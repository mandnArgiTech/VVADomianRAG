...
next2:      *(ckt->CKTstate0 + here->DIOvoltage) = vd;
            ...
            *(ckt->CKTstate0 + here->DIOconduct) = gd;
             ...
             *   load current vector
             */
            if (ngspice_diag_wants_device() && isfinite(vd) && isfinite(cd) && isfinite(gd)) {
                double ieq = cd - gd * vd;
                const char *nm = (here->DIOname) ? (const char *) here->DIOname : "";

                if (isfinite(ieq))
                    ngspice_diag_emit_device_dio(nm, vd, cd, gd, ieq);
            }
            cdeq=cd-gd*vd;
